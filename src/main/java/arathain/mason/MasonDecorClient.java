...
import arathain.mason.item.ScytheItemRenderer;
...
public class MasonDecorClient implements ClientModInitializer {
    ...
    public void onInitializeClient(ModContainer mod) {
        ...
        Identifier glaiveId = Registries.ITEM.getId(MasonObjects.GLAIVE);
        GlaiveItemRenderer glaiveItemRenderer = new GlaiveItemRenderer(glaiveId);
        ResourceLoader.get(ResourceType.CLIENT_RESOURCES).registerReloader(glaiveItemRenderer);
        ResourceLoader.get(ResourceType.CLIENT_RESOURCES).addReloaderOrdering(ResourceReloaderKeys.Client.MODELS, glaiveItemRenderer.getQuiltId());
        BuiltinItemRendererRegistry.INSTANCE.register(MasonObjects.GLAIVE, glaiveItemRenderer);
        ModelLoadingPlugin.register((manager) -> {
            manager.addModels(
                    new ModelIdentifier(glaiveId.getNamespace(), glaiveId.getPath() + "_gui", "inventory"),
                    new ModelIdentifier(glaiveId.getNamespace(), glaiveId.getPath() + "_handheld", "inventory")
            );
        });

        Identifier scytheId = Registries.ITEM.getId(MasonObjects.SCYTHE);
        ...
        BuiltinItemRendererRegistry.INSTANCE.register(MasonObjects.SCYTHE, scytheItemRenderer);
        ModelLoadingPlugin.register((manager) -> {
            manager.addModels(
