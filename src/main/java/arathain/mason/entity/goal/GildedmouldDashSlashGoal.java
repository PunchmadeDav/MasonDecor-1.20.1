...
public class GildedmouldDashSlashGoal extends Goal {
    private final GildedmouldEntity mould;

    public GildedmouldDashSlashGoal(GildedmouldEntity entity) {
        this.mould = entity;
        ...
        if (ticks == 10 || ticks == 13 || ticks == 15) {
            ...
            List<LivingEntity> entities = this.mould.getWorld().getEntitiesByClass(LivingEntity.class, this.mould.getBoundingBox().expand(4.0, 3.0, 4.0), (livingEntity) -> {
                ...
                if (livingEntity != this.mould && livingEntity != this.mould.getOwner()) {
                    label19: {
                        if (livingEntity instanceof GildedmouldEntity) {
                            GildedmouldEntity smould = (GildedmouldEntity)livingEntity;
