...
public class GildedmouldEntity extends HostileEntity implements TameableHostileEntity, GeoAnimatable {
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(OWNER_UUID, Optional.of(UUID.fromString("d3f2d795-04f0-4ebf-a398-93f3d4e7c33a")));
    }
    ...
    private void cycleActionState(PlayerEntity player) {
        if(getActionState() == 1) {
            setActionState(2);
            player.sendMessage(Text.translatable("amogus", getWorld().getRegistryKey().getValue().getPath()).setStyle(Style.EMPTY.withColor(Formatting.GOLD).withObfuscated(true).withFont(new Identifier("minecraft", "default"))), true);
        } else if(getActionState() == 0) {
            setActionState(1);
            player.sendMessage(Text.translatable("info.tot.mould_activate", getWorld().getRegistryKey().getValue().getPath()).setStyle(Style.EMPTY.withColor(Formatting.YELLOW)), true);
        } else if(getActionState() == 2) {
            setActionState(0);
            player.sendMessage(Text.translatable("info.tot.mould_deactivate", getWorld().getRegistryKey().getValue().getPath()).setStyle(Style.EMPTY.withColor(Formatting.BLACK)), true);
        }
    }
    @Override
    ...
    protected void playStepSound(BlockPos pos, BlockState state) {
        if(getWorld() instanceof ServerWorld server)
            server.getServer().getPlayerManager().sendToAround(null, this.getX(), this.getY(), this.getZ(), 32.0, server.getRegistryKey(), new PlaySoundFromEntityS2CPacket(Holder.createDirect(SoundEvents.ENTITY_IRON_GOLEM_STEP), this.getSoundCategory(), this, 32.0F, 1.0F, 69L));
    }
